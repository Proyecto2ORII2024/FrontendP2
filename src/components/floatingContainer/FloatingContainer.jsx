...
function FloatingContainer({open, setOpen, children, bttType = 0}){
  ...
  return(
      <div className={`fixed inset-0 bg-black bg-opacity-30 h-screen flex flex-col justify-center items-center transition-opacity duration-300 ${open ? 'opacity-100 visible' : 'opacity-0 invisible'}`} style={{zIndex:150}}>
        ...
        {bttType === 0 && (
          <button className='absolute top-3 right-5' type='button' onClick={() => setOpen(false)}>
            {XBtt}
          </button>
        )}
          {children}
        ...
        {bttType === 1 && (
          <MainButton text={'Entendido'} bgColor={'primary-dark'} hoverBg={'primary-light'} textColor={'white'} onClick={() => {setOpen(false);     console.log('Al darle click: '+ open);}} className={'my-3'}/>
        )}
      </div>
  );
}
