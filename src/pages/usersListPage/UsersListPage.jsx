...
function UsersListPage() {
  ...
  const handleEditClick = (e) => {
    ...
    if ((isEditing.state === false, isEditing.index === -1)) {
      estudiantes.map((estudiante) =>
        estudiante.Id === id
          ? setValues({ correo: estudiante.Correo, rol: estudiante.Rol })
          : null
      );
  ...
  const handleSaveClick = (e) => {
    ...
    if (esCorreoValido(values.correo) && role.includes(values.rol)) {
      ...
      setEstudiantes(
        estudiantes.map((estudiante) =>
          estudiante.Id === id
            ? { ...estudiante, Correo: values.correo, Rol: values.rol }
            : estudiante
        )
      );
  ...
  const handleCheckboxChange = (e) => {
    ...
    setEstudiantes(
      estudiantes.map((estudiante) =>
        estudiante.Id === id
          ? { ...estudiante, Active: !estudiante.Active }
          : estudiante
      )
    );
  };
    ...
    <AdminLayout>
      <main>
        ...
        <section className="flex justify-between items-center flex-col md:flex-row">
          ...
          <section className="w-full flex gap-3 md:gap-0 mb-5 flex-col md:flex-row justify-between items-center">
